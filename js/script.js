$(document).ready( function() {
	...
	$.getJSON("https://raw.githubusercontent.com/aryanlall11/aryanlall11.github.io/main/contact.json", function(contact_details){
		...
		if (contact_details["email"]){
			...
			for (var x in itr_obj)
				itr_obj[x].innerHTML = itr_obj[x].innerHTML+" Email";
		}
		else{
			$(".email").hide();
		}
		...
		if (contact_details["facebook"]){
			var itr_obj = $(".facebook");
			for (var x in itr_obj)
				itr_obj[x].href = contact_details["facebook"];
		}
		else{
			$(".facebook").hide();
		}
